...
namespace FastCore.Redis
{
    public class RedisCache : CacheBase
    {
        ...
        {
            var objbyte = _database.StringGet(GetLocalizedRedisKey(key));
            return objbyte.HasValue ? Deserialize<T>(objbyte) : default;
        }
        ...
        public override void SetAdd(string key, string value)
        {
            _database.SetAdd(GetLocalizedRedisKey(key), value);
        }

        public override bool SetContains(string key, string value)
        {
            return _database.SetContains(GetLocalizedRedisKey(key), value);
        }
        ...
        protected T Deserialize<T>(RedisValue objbyte)
        {
